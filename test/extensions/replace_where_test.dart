...
void main() {
  group('replaceWhere', () {
    test('replace elements matching a condition', () {
      ...
      final result = list.replaceWhere(6, where: (n) => n.isEven);
      expect(result, orderedEquals(<int>[1, 6, 3, 6, 5]));

      final result2 = list.replaceWhere(6, where: (n) => n % 4 == 0);
      expect(result2, orderedEquals(<int>[1, 2, 3, 6, 5]));

      final result3 = list.replaceWhere(6, where: (n) => n.isNegative);
